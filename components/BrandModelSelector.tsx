...
export default function BrandModelSelector({ productName, productCode, productSlug, productImage = null, models }: { productName: string; productCode: string; productSlug: string; productImage?: string | null; models: PowerToolModel[] }) {
  ...
  const quoteVariants = selected
    ? {
        Brand: selected.brand,
        Model: selected.name,
        ...(selected.series ? { Series: selected.series } : {}),
      }
    : {};
  ...
  return <section className="mb-8 overflow-hidden rounded-3xl border border-gray-200 bg-white">
    ...
    <div className="space-y-5 p-5 sm:p-6">
      ...
      {selected && <div className="rounded-2xl bg-red-50 p-4"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-red-700">Selected Product</p><h3 className="mt-2 text-lg font-bold text-gray-900">{selected.name}</h3><p className="text-sm font-medium text-red-700">{selected.brand}</p>{selected.series && <p className="mt-2 text-sm text-gray-600">Series: {selected.series}</p>}{Object.keys(selected.specifications).length > 0 && <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">{Object.entries(selected.specifications).map(([key, value]) => <div key={key}><dt className="font-semibold text-gray-700">{key}</dt><dd className="text-gray-600">{value}</dd></div>)}</dl>}<div className="mt-5 flex flex-wrap gap-3"><AddToQuoteButton productName={productName} productCode={productCode} productSlug={productSlug} productImage={productImage} variants={quoteVariants} /><a href={`https://wa.me/919626652275?text=${encodeURIComponent(message)}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center rounded-xl bg-green-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-green-700">WhatsApp for Quote</a></div></div>}
    </div>
  </section>;
}
