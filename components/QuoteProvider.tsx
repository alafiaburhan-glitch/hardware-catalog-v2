...
export type QuoteItem = { key: string; productName: string; productCode: string; productSlug?: string; productImage?: string | null; quantity: number; variants: Record<string, string> };
              ...
              <table className="w-full min-w-[640px] border-collapse text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-200">
                  {items.map((item, index) => <tr key={item.key} className="align-top odd:bg-white even:bg-slate-50/70">
                    ...
                    <td className="px-4 py-4"><Link href={item.productSlug ? `/products/${item.productSlug}` : `/search?q=${encodeURIComponent(item.productCode)}`} onClick={() => setOpen(false)} className="group flex items-center gap-3 rounded-xl focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-red-700">{item.productImage ? <Image src={item.productImage} alt="" width={56} height={56} className="h-14 w-14 shrink-0 rounded-xl border border-slate-200 bg-white object-contain p-1 transition group-hover:border-red-300" /> : <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 text-[10px] font-bold uppercase text-slate-400 transition group-hover:border-red-300">No image</div>}<div><p className="font-bold text-slate-900 transition group-hover:text-red-700 group-hover:underline">{item.productName}</p><p className="mt-1 text-xs font-medium text-slate-500">Code: {item.productCode}</p></div></Link></td>
